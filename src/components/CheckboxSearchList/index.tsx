...
function CheckboxSearchList({ initialList, onToggleCheckbox }: CheckboxSearchListProps) {
          ...
          return (
            ...
            >
              <ListItemButton role={undefined} onClick={() => handleToggle(item.id)} dense>
                ...
                <ListItemText id={labelId} primary={`${item.name} - ${item.email}`} />
              </ListItemButton>
            </ListItem>
          );
        })}
...
export function useCheckboxSearchList({ initialList }: { initialList: any[] }) {
  ...
  const Component = () => {
    return <CheckboxSearchList initialList={initialList} onToggleCheckbox={onToggleCheckbox} />;
  };
  return {
    values,
